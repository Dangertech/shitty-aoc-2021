...
bool intinvec(int this_int, std::vector <int> vec)
{
...
int main()
{
	...
	*/
	...
	for (int drawn_num=0; drawn_num < call_numbers.size(); drawn_num++)
	{
		...
		// Checks if a table in bingo_tables is new in comparison to before and pushes
		// it to the last place
		// This effectively sorts the boards by their winning order
		for (int i = 0; i<bingo_tables.size(); i++)
		{
			if (!intinvec(bingo_tables[i], sorted_tables))
				sorted_tables.push_back(bingo_tables[i]);
		}
		std::cout << "Recursion " << drawn_num << "; ";
		print_tables(94);
	}
	for (int i = 0; i<sorted_tables.size(); i++)
		std::cout << sorted_tables[i] << " ";
	std::cout << std::endl;
